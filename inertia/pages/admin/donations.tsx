...
const initialData: Donation[] = [
  {
    id: 1,
    donateur: 'Anonyme',
    montant_nature: '150 USD',
    type: 'card',
    date: '05 Mai 2025',
    statut: 'Complété',
    notes: 'Don via carte bancaire',
  },
  {
    id: 2,
    donateur: 'Famille Kasongo',
    montant_nature: '10 chaises plastiques',
    type: 'en nature',
    date: '03 Mai 2025',
    statut: 'En attente',
    notes: 'À récupérer au secrétariat',
  },
  {
    id: 3,
    donateur: 'Paul Ilunga',
    montant_nature: '500 USD',
    type: 'virement',
    date: '01 Mai 2025',
    statut: 'Complété',
    notes: 'Virement Equity BCDC',
  },
  {
    id: 4,
    donateur: 'Anonyme',
    montant_nature: '50 000 CDF',
    type: 'e-money',
    date: '28 Avr 2025',
    statut: 'Annulé',
    notes: 'Paiement mobile money échoué',
  },
  {
    id: 5,
    donateur: 'Sarah Lukusa',
    montant_nature: 'Instruments de musique',
    type: 'en nature',
    date: '25 Avr 2025',
    statut: 'Complété',
    notes: 'Remis lors du culte',
  },
]
...
export default function AdminDonations() {
  ...
  const filtered = donations.filter((d) => {
    const matchSearch =
      d.donateur.toLowerCase().includes(search.toLowerCase()) ||
      d.montant_nature.toLowerCase().includes(search.toLowerCase())
  ...
  function openEdit(d: Donation) {
    ...
    setForm({
      donateur: d.donateur,
      montant_nature: d.montant_nature,
      type: d.type,
      date: d.date,
      statut: d.statut,
      notes: d.notes,
  ...
  function openDelete(d: Donation) {
    setSelected(d)
    setModal('delete')
  }
  function closeModal() {
    ...
    <>
      ...
      <AdminLayout title="Gestion des dons">
        ...
        <div className="flex flex-wrap gap-3 mb-6 no-print">
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors border border-slate-700"
          >
            <Printer size={16} /> Imprimer
          </button>
          <button
            onClick={openAdd}
            className="flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors"
          >
              ...
              <thead>
                <tr className="bg-slate-900/50 border-b border-slate-800">
                  {['Donateur', 'Montant / Nature', 'Type', 'Date', 'Statut'].map((h) => (
                    <th
                      key={h}
                      className="text-left text-slate-400 font-medium px-6 py-3 text-xs uppercase tracking-wider whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
                  <th className="text-left text-slate-400 font-medium px-6 py-3 text-xs uppercase tracking-wider whitespace-nowrap no-print">
                    Actions
                  </th>
                </tr>
              </thead>
                  ...
                  <tr key={d.id} className="hover:bg-slate-800/30 transition-colors group">
                    <td className="px-6 py-4">
                      <div>
                        ...
                        {d.notes && (
                          <p className="text-slate-500 text-xs mt-0.5 truncate max-w-[150px]">
                            {d.notes}
                          </p>
                        )}
                      </div>
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
                <h3 className="text-white font-bold">
                  {modal === 'add' ? 'Enregistrer un don' : 'Modifier le don'}
                </h3>
              ...
              <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-800">
                ...
                <button
                  onClick={handleSave}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm bg-primary hover:bg-primary-dark text-white font-semibold transition-colors"
                >
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-4">
              ...
              <div className="text-center">
                ...
                <p className="text-slate-400 text-sm mt-1">
                  Êtes-vous sûr de vouloir supprimer la trace de ce don de{' '}
                  <strong className="text-white">{selected.montant_nature}</strong> par{' '}
                  <strong className="text-white">{selected.donateur}</strong> ?
                </p>
              </div>
