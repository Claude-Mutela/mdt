      ...
      <AdminLayout title="Gestion des membres">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                ...
                <tr key={m.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${m.statut === 'actif' ? 'bg-green-500' : 'bg-red-500'}`}
                        />
                        <span
                          className={`text-[10px] uppercase font-bold tracking-tighter ${m.statut === 'actif' ? 'text-green-500' : 'text-red-500'}`}
                        >
                          {m.statut}
                        </span>
                      </div>
          ...
          {members.data.length === 0 && (
            <div className="p-12 text-center text-slate-500">Aucun membre trouvé.</div>
          )}
        </div>
              ...
              >
                  ...
                  <div className="flex items-center gap-6 bg-slate-800/20 border border-slate-800/80 p-4 rounded-2xl">
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <Upload size={13} />{' '}
                          {form.data.coverImg ? 'Changer' : 'Sélectionner une photo'}
                        </button>
                        {form.data.coverImg && (
                          <button
                            type="button"
                      ...
                      <input
                        ...
                        accept="image/*"
                        onChange={(e) =>
                          form.setData('coverImg', e.target.files ? e.target.files[0] : null)
                        }
                      />
                      <p className="text-[10px] text-slate-500">
                        {form.data.coverImg
                          ? form.data.coverImg.name
                          : 'JPG, PNG ou WEBP. Max 2 Mo.'}
                      </p>
                    </div>
                  </div>
                  {form.errors.coverImg && (
                    <p className="text-red-400 text-[10px] italic">{form.errors.coverImg}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    />
                    {form.errors.email && (
                      <p className="text-red-400 text-[10px] mt-1 italic">{form.errors.email}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 uppercase tracking-wider font-semibold">
                    ...
                    />
                    {form.errors.phone && (
                      <p className="text-red-400 text-[10px] mt-1 italic">{form.errors.phone}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    {form.errors.gender && (
                      <p className="text-red-400 text-[10px] mt-1 italic">{form.errors.gender}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 uppercase tracking-wider font-semibold">
                    ...
                    {form.errors.statut && (
                      <p className="text-red-400 text-[10px] mt-1 italic">{form.errors.statut}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 uppercase tracking-wider font-semibold">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-2 rounded-xl text-sm bg-primary hover:bg-primary-dark text-white font-semibold transition-colors disabled:opacity-50"
                  >
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-5">
              ...
              <div className="space-y-4">
                <p className="text-sm text-slate-400">
                  Choisissez les données que vous souhaitez imprimer.
                </p>
                  ...
                  <label className="flex flex-col gap-2 p-3 border border-slate-700 rounded-xl cursor-pointer hover:bg-slate-800 transition-colors">
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        onChange={() => {
                          ...
                          if (!printMinistryId) {
                            setPrintMinistryId(
                              ministries
                                .filter((m) => !m.name.toLowerCase().includes('test'))[0]
                                ?.id.toString() || '1'
                            )
                          }
                        }}
                      ...
                      <div>
                        ...
                        <div className="text-slate-500 text-xs">
                          Uniquement les membres d'un département.
                        </div>
                      </div>
                    </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-800">
                ...
                <button
                  onClick={handlePrint}
                  className="flex-1 py-2.5 rounded-xl bg-primary hover:bg-primary-dark text-white text-sm font-bold transition-colors shadow-lg shadow-primary/20"
                >
                  Générer le document
                </button>
              </div>
