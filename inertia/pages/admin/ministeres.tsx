...
export default function AdminMinisteres({ ministries }: { ministries: Ministry[] }) {
  ...
  const paginatedMinistries = (ministries || []).slice(
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Départements et ministères spécifiques de l'église.
            </p>
          </div>
          <button
            onClick={openAddMin}
          ...
          <>
            <div className="grid md:grid-cols-2 xl:grid-cols-2 gap-5 mb-8">
              {paginatedMinistries.map((m) => (
                <div
                  key={m.id}
                  className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden group hover:border-slate-600 transition-colors shadow-xl"
                >
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-3">
                        {m.coverImg ? (
                          ...
                          <div
                            className={`w-12 h-12 ${m.badgeColor || 'bg-slate-500'} rounded-xl flex items-center justify-center text-white font-black text-lg shrink-0 shadow-sm`}
                          >
                            {(m.name || 'M')[0]}
                          </div>
                        )}
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
                <h3 className="text-white font-bold">
                  {minModal === 'add' ? 'Ajouter un ministère' : 'Modifier le ministère'}
                </h3>
                <button
                  onClick={() => setMinModal(null)}
              ...
              >
                <div className="grid grid-cols-2 gap-5">
                  ...
                  <div className="col-span-2 md:col-span-1">
                    ...
                    />
                    {minForm.errors.name && (
                      <p className="text-red-400 text-xs mt-1 italic">{minForm.errors.name}</p>
                    )}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    >
                      <input
                        ...
                        className="hidden"
                        onChange={(e) =>
                          minForm.setData('coverImg', e.target.files ? e.target.files[0] : null)
                        }
                      />
                      <Upload
                        size={24}
                  ...
                  {minForm.errors.coverImg && (
                    <p className="text-red-400 text-[10px] italic">{minForm.errors.coverImg}</p>
                  )}
                </div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {COULEURS.map(({ label, value }) => (
                      <button
                        ...
                        title={label}
                        type="button"
                        onClick={() => minForm.setData('badgeColor', value)}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    />
                    {minForm.errors.content && (
                      <p className="text-red-400 text-xs mt-1 italic">{minForm.errors.content}</p>
                    )}
                  </div>
                </div>
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-4">
              ...
              <div className="text-center">
                ...
                <p className="text-slate-400 text-sm mt-1">
                  ...
                  <strong className="text-white">{selectedMin.name}</strong> ?
                </p>
              </div>
