  ...
  return (
    <>
      <Head title="Nos Ministères — Phila MDT">
        <meta
          name="description"
          content="Découvrez les différents ministères de la Phila Maison de Témoignages (MDT) et trouvez votre place pour servir et grandir dans la foi."
        />
      </Head>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {ministries.map((min, idx) => (
                <div
                  key={min.id ?? idx}
                  className="group flex flex-col rounded-2xl border border-slate-100 overflow-hidden hover:shadow-2xl transition-all duration-500 bg-white"
                >
                  <div className="relative h-48 overflow-hidden">
                    {min.img ? (
                      ...
                      <div
                        className={`w-full h-full flex items-center justify-center text-white text-4xl font-black ${min.color || 'bg-slate-400'}`}
                      >
                    ...
                    {min.tag && (
                      <span
                        className={`absolute bottom-4 left-4 ${min.color || 'bg-slate-500'} text-white text-[10px] font-black px-3 py-1 rounded-lg uppercase tracking-widest`}
                      >
                        {min.tag}
                      </span>
                    )}
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <p className="text-sm text-slate-500 leading-relaxed mb-6">
                      {min.desc || 'Aucune description disponible.'}
                    </p>
                    {min.slug && (
                      <Link
                        href={`/ministeres/${min.slug}`}
                        className="mt-auto flex items-center gap-2 text-primary text-sm font-bold hover:gap-4 transition-all"
                      >
                        En savoir plus
                        <ArrowRight size={14} />
                      </Link>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {responsibles.map((person, idx) => (
                  <div
                    key={person.id ?? idx}
                    className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-xl transition-all duration-300 group text-center flex flex-col items-center p-8"
                  >
